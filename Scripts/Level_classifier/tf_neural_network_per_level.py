...
def train_level(data, targets, level, n_hidden, n_nodes, activations, n_epochs, batch_size, prev_class="None", random_state=0):
    ...
    nn.fit_generator(generate_batch(data, targets, train, batch_size, n_epochs, random_state), round(len(train)/batch_size), epochs=n_epochs)
...
def generate_batch(data, targets, train, batch_size, epochs=1, random_state=0, keep_duplicate=True):
    ...
    for i in range(epochs):
        ...
        for j in range(math.ceil(len(train)/batch_size)):
            ...
            y_batch = [targets[x] for x in train_rearr[j*batch_size:(j+1)*batch_size]]
